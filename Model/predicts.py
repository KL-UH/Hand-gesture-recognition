...
while True:
    ...
    prediction = {
        'Headache - Tylenol': result[0][0],
        'Fever - Ibuprofen': result[0][1],
        'Nausia - Palanosteron': result[0][2],
        'Stomach Pain - Nexium': result[0][3],
        'BackPain - Diclofenac': result[0][4],
        'Cough - Mucinex': result[0][5],
        'Sore Throth - benadryl': result[0][6],
        'Acne - Retin-A': result[0][7],
        'Depression - sertraline ': result[0][8]}
    # Sorting based on top prediction
    prediction = sorted(prediction.items(), key=operator.itemgetter(1), reverse=True)

    # Displaying the predictions
    cv2.putText(frame, prediction[0][0], (10, 60), cv2.FONT_HERSHEY_PLAIN, 3, (255, 255, 255), 5)
    cv2.imshow("Frame", frame)
